...
function buildPDFHTML(
  ...
  playerList: Array<{ name: string; belt_rank: string; todayStatus: string; timeIn: string | null }>,
  weeklyHistory: Array<{ date: string; present: number; late: number; absent: number; total: number }>
): string {
  const sc = (s: string) => s === "present" ? "#10B981" : s === "late" ? "#F59E0B" : "#EF4444";
  const sl = (s: string) => s === "present" ? "Present" : s === "late" ? "Late" : "Absent";
  const fd = (d: string) => new Date(d + "T00:00:00").toLocaleDateString("en-US", { month: "short", day: "numeric" });
  const ft = (iso: string | null) => {
    if (!iso) return "—";
...
h1{color:#D32F2F;font-size:22px;margin:0 0 4px;}
h2{font-size:15px;margin:20px 0 8px;color:#1E40AF;border-bottom:2px solid #1E40AF;padding-bottom:4px;}
.sub{color:#555;font-size:12px;margin:0 0 16px;}
.sumbox{display:inline-block;width:22%;text-align:center;border-radius:8px;padding:10px;margin-right:2%;}
.num{font-size:26px;font-weight:bold;margin:0;}
.lbl{font-size:11px;color:#666;margin:4px 0 0;}
table{width:100%;border-collapse:collapse;font-size:12px;margin-top:4px;}
th{background:#F3F4F6;padding:8px;text-align:left;font-size:11px;color:#555;text-transform:uppercase;letter-spacing:.5px;}
...
<h1>Kagay-an Martial Arts School</h1>
<p class="sub">Attendance Report — ${todayLabel}</p>
<div>
  <span class="sumbox" style="background:#D1FAE5"><p class="num" style="color:#10B981">${summary.present}</p><p class="lbl">Present</p></span>
  <span class="sumbox" style="background:#FEF3C7"><p class="num" style="color:#F59E0B">${summary.late}</p><p class="lbl">Late</p></span>
  <span class="sumbox" style="background:#FEE2E2"><p class="num" style="color:#EF4444">${summary.absent}</p><p class="lbl">Absent</p></span>
  <span class="sumbox" style="background:#EFF6FF"><p class="num" style="color:#1E40AF">${summary.total}</p><p class="lbl">Total Active</p></span>
</div>
<h2>Today's Attendance Detail</h2>
<table><thead><tr><th>#</th><th>Name</th><th>Belt</th><th>Status</th><th>Time In</th></tr></thead><tbody>
${playerList.map((p, i) => `<tr><td>${i + 1}</td><td>${p.name}</td><td>${p.belt_rank}</td><td><span class="badge" style="background:${sc(p.todayStatus)}22;color:${sc(p.todayStatus)}">${sl(p.todayStatus)}</span></td><td>${ft(p.timeIn)}</td></tr>`).join("")}
</tbody></table>
<h2>Weekly Summary (Past 7 Days)</h2>
<table><thead><tr><th>Date</th><th>Present</th><th>Late</th><th>Absent</th><th>Total Active</th></tr></thead><tbody>
${weeklyHistory.map((d) => `<tr><td>${fd(d.date)}</td><td style="color:#10B981;font-weight:bold">${d.present}</td><td style="color:#F59E0B;font-weight:bold">${d.late}</td><td style="color:#EF4444;font-weight:bold">${d.absent}</td><td>${d.total}</td></tr>`).join("")}
</tbody></table>
<p class="footer">Generated by Kagay-an Martial Arts School App · ${new Date().toLocaleString()}</p>
</body></html>`;
}
...
export default function DashboardScreen() {
  ...
  const { user, updateProfilePhoto, staffBirthdaysToday } = useAuth();
  ...
  const birthdayPeople = useMemo(() => {
    ...
    const bPlayers = players.filter((p) => p.birthdate && p.birthdate.slice(5) === mmdd).map((p) => p.name.split(" ")[0]);
    return [...bPlayers, ...staffBirthdaysToday.map((n) => n.split(" ").slice(0, 2).join(" "))];
  }, [players, staffBirthdaysToday]);
  ...
  const weeklyHistory = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const presentIds = new Set(dayRecs.map((r) => r.playerId));
      const present = dayRecs.filter((r) => r.status === "present").length;
      const late = dayRecs.filter((r) => r.status === "late").length;
      const activePlayers = players.filter((p) => p.membership_status !== "inactive");
      const absent = activePlayers.filter((p) => !presentIds.has(p.id)).length;
      days.push({ date: dateStr, present, late, absent, total: activePlayers.length });
    }
    return days;
  }, [records, players]);
  ...
  const handleExportPDF = async () => {
    ...
    const html = buildPDFHTML(todayLabel, attendanceStats, playerListToday, weeklyHistory);
    try {
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        {birthdayPeople.length > 0 && (
          <View style={[styles.birthdayCard, { backgroundColor: "#FEF3C7", borderColor: "#F59E0B" }]}>
            <Text style={styles.birthdayCake}>🎂</Text>
            <View style={{ flex: 1 }}>
              <Text style={styles.birthdayTitle}>Birthday Today!</Text>
              <Text style={styles.birthdayNames} numberOfLines={2}>
                {birthdayPeople.join(", ")} — Happy Birthday! 🎉
              </Text>
            </View>
          </View>
        )}
        ...
        <View style={[styles.section, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={styles.sectionHeaderRow}>
            ...
            >
              <Feather name="download" size={13} color="#1E40AF" />
...
const styles = StyleSheet.create({
  ...
  birthdayCard: { flexDirection: "row", alignItems: "center", gap: 12, borderWidth: 1.5, borderRadius: 14, padding: 14 },
  birthdayCake: { fontSize: 28 },
  birthdayTitle: { color: "#92400E", fontSize: 14, fontFamily: "Inter_700Bold" },
  birthdayNames: { color: "#78350F", fontSize: 12, fontFamily: "Inter_500Medium", marginTop: 2 },
  sectionHeaderRow: { flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", marginBottom: -4 },
  pdfBtn: { flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "#EFF6FF", borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6, borderWidth: 1, borderColor: "#BFDBFE" },
  pdfBtnText: { color: "#1E40AF", fontSize: 12, fontFamily: "Inter_600SemiBold" },
